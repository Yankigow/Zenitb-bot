...
if(!muteRoleFetch) return message.channel.send('Bu sunucuda müte rolü ayarlı değil ayarlamak için `z!muterol` yada sizin için oluşturmamızı istiyorsanuz `z!muterol create <müteroladı>` yazabilirsiniz.');

if(!args[0]) return message.channel.send(`\`\`\`${message.content.split('mute')[0]}mute [kişi] [süre] [sebeb]
      ^^^^^^^^
Eksik kelime girdiniz. örnek **z!mute @ZeZe 2.5 1d deneme**\`\`\``);
...
if(ms(zaman) >= 2147483647) return message.channel.send('24 gün boyunca en fazla birini sessize alabilirsiniz.');// Burayla fazla oynamayın hata alırsınız
